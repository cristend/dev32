...
from cartmigration.libs.utils import (
    response_success, response_error, to_str, to_decimal, to_int, duplicate_field_value_from_list,  # noqa
    response_warning, get_row_from_list_by_field, get_list_from_list_by_field, get_current_time, get_value_by_key_in_dict, to_len, re  # noqa
)
...
class LeCartCustom(LeBasecart):
    ...
    def display_import_source(self):
        if self._notice['config']['add_new']:
            ...
            if recent:
                types = ['taxes', 'manufacturers', 'categories', 'attributes', 'products',              # noqa
                         'customers', 'orders', 'reviews', 'pages', 'blogs', 'coupons', 'cartrules']    # noqa
                for _type in types:
                    self._notice['process'][_type]['id_src'] = recent['process'][_type]['id_src']  # noqa
        ...
        queries = {
            # 'taxes': {
            # 	'type': 'select',
            # 	'query': "SELECT COUNT(1) AS count FROM  _DBPRF_StateTaxStates WHERE id > " + to_str(self._notice['process']['taxes']['id_src']), # noqa
            # },
            # 'manufacturers': {
            # 	'type': 'select',
            # 	'query': "SELECT COUNT(1) AS count  FROM _DBPRF_manufacturers WHERE manufacturers_id > " + to_str(self._notice['process']['manufacturers']['id_src']), # noqa
            # },
            # 'categories': {
            #     'type': 'select',
            #     'query': "SELECT COUNT(1) AS count FROM  _DBPRF_categories WHERE categories_id > " + to_str(self._notice['process']['categories']['id_src']),  # noqa
            # },
            # 'products': {
            #     'type': 'select',
            #     'query': "SELECT COUNT(1) AS count FROM  _DBPRF_products WHERE products_id > " + to_str(self._notice['process']['products']['id_src']),  # noqa
            # },
            # 'customers': {
            #     'type': 'select',
            #     'query': "SELECT COUNT(1) AS count FROM  _DBPRF_customers WHERE customers_id > " + to_str(self._notice['process']['customers']['id_src']),  # noqa
            # },
            'orders': {
                'type': 'select',
                'query': "SELECT COUNT(1) AS count FROM  _DBPRF_orders WHERE  orders_id > " + to_str(self._notice['process']['orders']['id_src']),  # noqa
            },
            # 'reviews': {
            # 	'type': 'select',
            # 	'query': "SELECT COUNT(1) AS count FROM _DBPRF_reviews WHERE reviews_id > " + to_str(self._notice['process']['reviews']['id_src']), # noqa
            # },
        }
    ...
    def get_taxes_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': "SELECT * FROM _DBPRF_StateTaxStates WHERE id > " + to_str(  # noqa
                    id_src) + " ORDER BY id ASC LIMIT " + to_str(limit)
        }
        taxes = self.get_connector_data(self.get_connector_url('query'), {
    ...
    def check_tax_import(self, convert, tax, taxes_ext):
        return True if self.get_map_field_by_src(self.TYPE_TAX, convert['id']) else False  # noqa
    ...
    def get_manufacturers_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': "SELECT * FROM _DBPRF_manufacturers WHERE manufacturers_id > " + to_str(  # noqa
                    id_src) + " ORDER BY manufacturers_id ASC LIMIT " + to_str(limit)  # noqa
        }
        manufacturers = self.get_connector_data(
            self.get_connector_url('query'), {'query': json.dumps(query)})
    ...
    def get_manufacturers_ext_export(self, manufacturers):
        ...
        manufacturer_ids = duplicate_field_value_from_list(
            manufacturers['data'], 'manufacturer_id')
        manufacturers_ext_queries = {
            'manufacturers_info': {
                'type': 'select',
                'query': "SELECT * FROM _DBPRF_manufacturers_info WHERE manufacturers_id IN " + self.list_to_in_condition(  # noqa
                    manufacturer_ids)
            }
        }
        manufacturers_ext = self.get_connector_data(url_query,
                                                    {'serialize': True, 'query': json.dumps(manufacturers_ext_queries)})  # noqa
    ...
    def convert_manufacturer_export(self, manufacturer, manufacturers_ext):
        ...
        manufacturer_data['thumb_image']['path'] = manufacturer['manufacturers_image']  # noqa

        for language_id, language_label in self._notice['src']['languages'].items():  # noqa
            manufacturer_language_data = dict()
            manufacturer_language_data['name'] = manufacturer['manufacturers_name']  # noqa
    ...
    def get_categories_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': (
                "SELECT * FROM  _DBPRF_categories WHERE categories_id > "
                + to_str(id_src)
                + " ORDER BY categories_id ASC LIMIT " + to_str(limit))
        }
    ...
    def get_categories_ext_export(self, categories):
        ...
        categories_ext_queries = {
            'categories_description': {
                ...
                'query': (
                    "SELECT * \
                    FROM  _DBPRF_categories_description \
                    WHERE categories_id IN "
                    + self.list_to_in_condition(category_ids))
            },
        }
        categories_ext = self.select_multiple_data_connector(
            categories_ext_queries)

        if not categories_ext or categories_ext['result'] != 'success':
            return response_warning()
        lang_ids = duplicate_field_value_from_list(
            categories_ext['data']['categories_description'], 'language_id')
        categories_ref_ext_queries = {
            'languages': {
                'type': 'select',
                'query': (
                    "SELECT * FROM  _DBPRF_languages WHERE languages_id IN "
                    + self.list_to_in_condition(lang_ids))
            },
        }

        categories_ref_ext = self.select_multiple_data_connector(
            categories_ref_ext_queries)
        if not categories_ref_ext or categories_ref_ext['result'] != 'success':
            return response_warning()
        categories_ext = self.sync_connector_object(
            categories_ext, categories_ref_ext)
    ...
    def convert_category_export(self, category, categories_ext):
        ...
        # code
        # parent
        category_parent = self.construct_category_parent()
        if (category['parent_id'] and
                to_decimal(category['parent_id']) != to_decimal(
            category['categories_id']) and
                to_int(category['parent_id']) > 2):
            parent_data = self.get_category_parent(category['parent_id'])
            if parent_data['result'] == 'success' and parent_data['data']:
                category_parent = parent_data['data']
            elif parent_data['result'] == 'success' and\
                    not parent_data['data']:
                category_parent['id'] = 0
            else:
                return response_error()
        else:
            category_parent['id'] = 0
        category_data['parent'] = category_parent
        category_data['active'] = True
        # thumb_image
        if category['categories_image']:
            label = category['categories_image'].split('.')[0]
        else:
            label = ''
        url = 'http://127.0.0.1/customcart/images/'
        path = category['categories_image']
        category_data['thumb_image'] = {
            'label': label,
            'url': url,
            'path': path,
        }
        # images
        category_data_ext = categories_ext['data']
        category_description = get_row_from_list_by_field(
            category_data_ext['categories_description'],
            'categories_id', category['categories_id'])
        category_data['name'] = (
            category_description['categories_name']
        ...
        languages = dict()
        if category_description:
            cat_langs = get_list_from_list_by_field(
                category_data_ext['languages'],
                'languages_id', category_description['language_id'])
            for cat_lang in cat_langs:
                lang = self.construct_category_lang()
                lang['name'] = cat_lang['name']
                lang['description'] = cat_lang['directory']
                languages[cat_lang['languages_id']] = lang
    ...
    def check_category_import(self, convert, category, categories_ext):
        id_imported = self.get_map_field_by_src(
            self.TYPE_CATEGORY, convert['categories_id'], convert['code'])
        return id_imported
    ...
    def get_products_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': ("SELECT * FROM  _DBPRF_products WHERE products_id > "
                      + to_str(id_src) + " ORDER BY products_id ASC LIMIT "
                      + to_str(limit))
        }

        products = self.get_connector_data(
            self.get_connector_url('query'), {'query': json.dumps(query)})
    ...
    def get_products_ext_export(self, products):
        ...
        product_ext_queries = {
            'products_description': {
                'type': "select",
                'query': "SELECT * FROM  _DBPRF_products_description \
                          WHERE products_id IN " + product_id_con,
            },
            'products_to_categories': {
                'type': "select",
                'query': "SELECT * FROM  _DBPRF_products_to_categories \
                          WHERE products_id IN  " + product_id_con + " \
                          ORDER BY 1",
            },
        }

        product_ext = self.select_multiple_data_connector(product_ext_queries)
    ...
    def convert_product_export(self, product, products_ext):
        ...
        if product['products_image']:
            label = product['products_image'].split('.')[0]
        else:
            label = ''
        url = 'http://127.0.0.1/customcart/images/'
        path = product['products_image']
        ...
        product_description = get_row_from_list_by_field(
            products_ext_data['products_description'],
        ...
        if product_description:
            ...
            product_description_link = (
                '<b>Nutrition_facts</b>: <a href="#">' +
                product_description['nutrition_facts_htm'] + '</a>')
            product_description_full = (
                product_description['products_description'] +
                product_description['other_ingredients'] +
                product_description['allergen_information'] +
                product_description_link)
            product_data['description'] = product_description_full
        # short description
        # meta title
        # meta keyword
        # meta description
        ...
        product_data['cost'] = product['products_wholesale_price']
        # special price
        # group price
        # tier price
        ...
        product_data['is_in_stock'] = (
            True if to_int(product['products_quantity']) > 0 else False)
        # tax
        product_data['manufacturer'] = {
            'id': product['manufacturers_id'],
            'code': None,
            'name': None, }
        # created_at
        # updated_at
        ...
        product_categories = get_list_from_list_by_field(
            products_ext_data['products_to_categories'],
            'products_id', product['products_id'])
        ...
        if product_description['language_id']:
            product_language_data = self.construct_product_lang()
            product_data['languages'] = {
                product_description['language_id']: product_language_data
            }

        option_data = self.construct_product_option()
        option_data['id'] = product['products_id']
        option_data['option_name'] = 'type'
        option_data['option_type'] = 'select'
        option_data['option_languages'] = product_data['languages']

        option_values = {
            'fl': product['products_quantity_fl'],
            'ga': product['products_quantity_ga'],
            'ct': product['products_quantity_ct'],
            'tx': product['products_quantity_tx'],
            'in': product['products_quantity_in']}

        for option, qty in option_values.items():
            option_value_data = self.construct_product_option_value()
            option_value_data['option_value_qty'] = qty
            option_value_data['id'] = product['products_id']
            option_value_data['option_value_name'] = option
    ...
    def check_product_import(self, convert, product, products_ext):
        return True if self.get_map_field_by_src(self.TYPE_PRODUCT, convert['products_id'], convert['code']) else False  # noqa
    ...
    def get_customers_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': "SELECT * FROM  _DBPRF_customers WHERE customers_id > " + to_str(id_src) + " ORDER BY customers_id ASC LIMIT " + to_str(limit)  # noqa
        }

        customers = self.get_connector_data(self.get_connector_url('query'), {
                                            'query': json.dumps(query)})
    ...
    def get_customers_ext_export(self, customers):
        customers_ids = duplicate_field_value_from_list(
            customers['data'], 'customers_id')
        customer_ext_queries = {
            'customers_info': {
                'type': 'select',
                'query': ("SELECT * FROM  _DBPRF_customers_info \
                          WHERE customers_info_id IN " +
        ...
        country_ids = duplicate_field_value_from_list(
            customers_ext['data']['address_book'], 'entry_country_id')
        zone_ids = duplicate_field_value_from_list(
            customers_ext['data']['address_book'], 'entry_zone_id')
        customers_ext_rel_queries = {
            'countries': {
                'type': 'select',
                'query': ("SELECT * FROM countries WHERE countries_id IN  " +
                          self.list_to_in_condition(country_ids)),
            },
            'zones': {
                'type': 'select',
                'query': ("SELECT * FROM  _DBPRF_zones WHERE zone_id IN " +
                          self.list_to_in_condition(zone_ids)),
            },
        }
        customers_ext_rel = self.select_multiple_data_connector(
            customers_ext_rel_queries)

        if not customers_ext_rel or customers_ext_rel['result'] != 'success':
            return response_error()

        customers_ext = self.sync_connector_object(
            customers_ext, customers_ext_rel)
    ...
    def convert_customer_export(self, customer, customers_ext):
        ...
        customers_info = get_row_from_list_by_field(
            ...
            'customer_info_id', customer['customers_id'])
        if customers_info:
            customer_data['is_subscribed'] = customers_info[
                'global_product_notifications']
            if 'status' in customers_info:
                customer_data['active'] = customers_info['status']
            else:
                customer_data['active'] = True
            customer_data['created_at'] = customers_info[
                'customers_info_date_account_created']
            customer_data['updated_at'] = customers_info[
                'customers_info_date_account_last_modified']
        # address
        address_books = get_list_from_list_by_field(
        ...
        if address_books:
            for address_book in address_books:
                ...
                address_data['address_1'] = get_value_by_key_in_dict(
                    address_book, 'entry_city', '')
                address_data['address_2'] = get_value_by_key_in_dict(
                    address_book, 'entry_stress_address', '')
                ...
                # country
                country = get_row_from_list_by_field(
                    customers_ext['data']['countries'], 'countries_id', address_book['entry_country_id'])  # noqa
                if country:
                    address_data['country']['id'] = country['countries_id']
                    address_data['country']['country_code'] = country['countries_iso_code_2']  # noqa
                    address_data['country']['name'] = country['countries_name']
                else:
                    address_data['country']['country_code'] = 'US'
                # zones
                zones = address_book['entry_zone_id']
                if zones:
                    state = get_row_from_list_by_field(
                        customers_ext['data']['zones'], 'zone_id', zones)
                    if state:
                        address_data['state']['id'] = state['zone_id']
                        address_data['state']['state_code'] = state[
                            'zone_code']
                        address_data['state']['name'] = state['zone_name']
                    elif address_book['entry_state']:
                        address_data['state']['state_code'] = address_book[
                            'entry_state']
                    else:
                        address_data['state']['state_code'] = 'AL'
                else:
                    address_data['state']['name'] = 'AL'
                # if address_book['address_book_id'] == customer['customers_default_address_id']: # noqa
                address_data['default']['billing'] = False
                address_data['default']['shipping'] = False
    ...
    def get_orders_main_export(self):
        ...
        query = {
            ...
            'query': ("SELECT * FROM  _DBPRF_orders WHERE orders_id > " +
                      to_str(id_src) + " ORDER BY orders_id ASC LIMIT " +
                      to_str(limit))
        }
        orders = self.get_connector_data(self.get_connector_url('query'), {
                                         'query': json.dumps(query)})
    ...
    def get_orders_ext_export(self, orders):
        order_ids = duplicate_field_value_from_list(
            orders['data'], 'orders_id')
        bil_country = duplicate_field_value_from_list(
            orders['data'], 'billing_country')
        delivery_country = duplicate_field_value_from_list(
            orders['data'], 'delivery_country')
        country_ids = set(bil_country + delivery_country)

        payment_zone = duplicate_field_value_from_list(
            orders['data'], 'billing_state')
        shipping_zone = duplicate_field_value_from_list(
            orders['data'], 'delivery_state')
        state_ids = set(payment_zone + shipping_zone)
        orders_ext_queries = {
            'orders_products': {
                'type': 'select',
                'query': "SELECT * FROM  _DBPRF_orders_products \
                    WHERE orders_id IN " + self.list_to_in_condition(order_ids)
            },
            'orders_total': {
                'type': 'select',
                'query': "SELECT * FROM  _DBPRF_orders_total \
                    WHERE orders_id IN " + self.list_to_in_condition(order_ids)
            },
            'countries': {
                'type': 'select',
                'query': ("SELECT * FROM  _DBPRF_countries \
                          WHERE countries_name IN " +
                          self.list_to_in_condition(country_ids))
            },
            'zones': {
                'type': 'select',
                'query': "SELECT * FROM  _DBPRF_zones \
                    WHERE zone_code IN " + self.list_to_in_condition(state_ids)
            }
        }
        orders_ext = self.select_multiple_data_connector(orders_ext_queries)
    ...
    def convert_order_export(self, order, orders_ext):
        ...
        order_product = get_row_from_list_by_field(
        ...
        # tax, discount, shipping, subtotal, total
        order_total = get_row_from_list_by_field(
            orders_ext['data']['orders_total'],
            'orders_id', order['orders_id']
        )
        if order_total:
            ot_tax = get_row_from_list_by_field(
                [order_total], 'class', 'ot_tax')
            ot_shipping = get_row_from_list_by_field(
                [order_total], 'class', 'ot_shipping')
            ot_subtotal = get_row_from_list_by_field(
                [order_total], 'class', 'ot_subtotal')
            ot_total = get_row_from_list_by_field(
                [order_total], 'class', 'ot_total')
            if ot_tax:
                order_data['tax']['title'] = ot_tax['title']
                order_data['tax']['amount'] = ot_tax['value']
                order_data['tax']['percent'] = order['products_tax']
            if ot_shipping:
        ...
        order_data['created_at'] = datetime.fromtimestamp(
            to_int(
                get_value_by_key_in_dict(
                    order, 'date_purchased', 0))).strftime('%Y-%m-%d %H:%M:%S')
        ...
        # fax
        # country
        countries = get_row_from_list_by_field(
            orders_ext['data']['countries'],
            'countries_name', order['delivery_country'])
        if countries:
            customer_address['country']['id'] = countries['countries_id']
            customer_address['country']['country_code'] = countries[
                'countries_iso_code_2']
            customer_address['country']['name'] = countries['countries_name']
        # state
        zone_name = order['delivery_state'].title()
        zone_code = order['delivery_state'].upper()
        zones = get_row_from_list_by_field(
            orders_ext['data']['zones'],
            'zone_name', zone_name)
        if not zones:
            zones = get_row_from_list_by_field(
                orders_ext['data']['zones'],
                'zone_code', zone_code)
        if zones:
            customer_address['state']['id'] = zones['zone_id']
            customer_address['state']['state_code'] = zones['zone_code']
            customer_address['state']['name'] = zones['zone_name']
        ...
        order_billing = {
            ...
            'last_name': '',
            'address_1': (order['billing_address1'] if
                          order['billing_address1'] else 'address1'),
            'address_2': (order['billing_address2'] if
                          order['billing_address2'] else 'address2'),
        ...
        # country
        billing_country = get_row_from_list_by_field(
            orders_ext['data']['countries'],
            'countries_name', order['billing_country'])
        if billing_country:
            order_billing['country']['id'] = billing_country['countries_id']
            order_billing['country']['country_code'] = billing_country[
                'countries_iso_code_2']
            order_billing['country']['name'] = billing_country[
                'countries_name']
        # state
        bill_zone_name = order['delivery_state'].title()
        bill_zone_code = order['delivery_state'].upper()
        bill_zones = get_row_from_list_by_field(
            orders_ext['data']['zones'],
            'zone_name', bill_zone_name)
        if not bill_zones:
            bill_zones = get_row_from_list_by_field(
                orders_ext['data']['zones'],
                'zone_code', bill_zone_code)
        if bill_zones:
            order_billing['state']['id'] = bill_zones['zone_id']
            order_billing['state']['state_code'] = bill_zones['zone_code']
            order_billing['state']['name'] = bill_zones['zone_name']
        ...
        order_delivery = {
            ...
            'last_name': '',
            'address_1': (order['delivery_address1'] if
                          order['delivery_address1'] else 'address1'),
            'address_2': (order['delivery_address2'] if
                          order['delivery_address2'] else 'address2'),
        ...
        # country
        delivery_country = get_row_from_list_by_field(
            orders_ext['data']['countries'],
            'countries_name', order['delivery_city'])
        if delivery_country:
            order_delivery['country']['id'] = delivery_country['countries_id']
            order_delivery['country']['country_code'] = delivery_country[
                'countries_iso_code_2']
            order_delivery['country']['name'] = delivery_country[
                'countries_name']
        # state
        if zones:
            order_delivery['state']['id'] = zones['zone_id']
            order_delivery['state']['state_code'] = zones['zone_code']
            order_delivery['state']['name'] = zones['zone_name']
        order_delivery = self._cook_shipping_address_by_billing(
            order_delivery, order_billing)
        ...
        for order_product in order_products:
            order_item_subtotal = (
                to_decimal(order_product['final_price']) *
                to_decimal(order_product['products_quantity']))
            order_item_tax = (
                to_decimal(order_item_subtotal) *
                to_decimal(order_product['products_tax']) / 100)
            order_item_total = (
                to_decimal(order_item_subtotal) + to_decimal(order_item_tax))
    ...
    def check_order_import(self, convert, order, orders_ext):
        return True if self.get_map_field_by_src(self.TYPE_ORDER, convert['id'], convert['code']) else False  # noqa
    ...
    def get_reviews_main_export(self):
        ...
        query = {
            'type': 'select',
            'query': "SELECT * FROM _DBPRF_reviews WHERE reviews_id > " + to_str(  # noqa
                    id_src) + " ORDER BY reviews_id ASC LIMIT " + to_str(limit)
        }
        reviews = self.get_connector_data(self.get_connector_url('query'), {
                                          'query': json.dumps(query)})
    ...
    def get_reviews_ext_export(self, reviews):
        url_query = self.get_connector_url('query')
        reviews_id = duplicate_field_value_from_list(
            reviews['data'], 'reviews_id')
        product_ids = duplicate_field_value_from_list(
            reviews['data'], 'products_id')
        reviews_ext_queries = {
            'products_description': {
                'type': 'select',
                'query': "SELECT products_id, language_id, products_name FROM _DBPRF_products_description WHERE products_id IN " + self.list_to_in_condition(  # noqa
                    product_ids)
            },
            'reviews_description': {
                'type': 'select',
                'query': "SELECT * FROM _DBPRF_reviews_description WHERE reviews_id IN " + self.list_to_in_condition(  # noqa
                    reviews_id)
            },
        }
    ...
    def convert_review_export(self, review, reviews_ext):
        ...
        review_description = get_row_from_list_by_field(
            reviews_ext['data']['reviews_description'], 'reviews_id', review['reviews_id'])  # noqa
        if not review_description:
            return response_warning(self.warning_import_entity('Review', review['reviews_id'], None, 'Review data not exists.'))  # noqa
        language_id = review_description['languages_id'] if review_description['languages_id'] else default_language  # noqa
        product_descriptions = get_list_from_list_by_field(
            reviews_ext['data']['products_description'], 'products_id', review['products_id'])  # noqa
        product_description = get_row_from_list_by_field(
            product_descriptions, 'language_id', language_id)
        if not product_description:
            product_description = get_row_from_list_by_field(
                product_descriptions, 'language_id', default_language)
        rv_status = {
            0: 2,  # pedding
            1: 1,  # approved
            3: 2  # not approved
        ...
        review_data['product']['name'] = product_description['products_name'] if product_description else ' '  # noqa
        ...
        review_data['status'] = get_value_by_key_in_dict(rv_status, to_int(
            review['reviews_status']), 1) if 'reviews_status' in review else 1
    ...
    def review_import(self, convert, review, reviews_ext):
        ...
        if convert['product']['id'] or convert['product']['code']:
            product_id = self.get_map_field_by_src(self.TYPE_PRODUCT, convert['product']['id'],  # noqa
                                                   convert['product']['code'])
        if not product_id:
            response_warning(
                'Review ' + to_str(convert['id']) + ' import false. Product does not exist!')  # noqa
        customer_id = 0
        if convert['customer']['id'] or convert['customer']['code']:
            customer_id = self.get_map_field_by_src(self.TYPE_CUSTOMER, convert['customer']['id'],  # noqa
                                                    convert['customer']['code'])  # noqa
            if not customer_id:
                customer_id = 0
        review_data = {
            'product_id': product_id,
            'customer_id': customer_id,
            'author': convert['customer']['name'] if convert['customer']['name'] else '',  # noqa
        ...
        review_id = self.import_review_data_connector(
            self.create_insert_query_connector('review', review_data), True, convert['id'])  # noqa
        if not review_id:
            response_warning(
                'review id ' + to_str(convert['id']) + ' import false.')
    ...
    def product_to_in_condition_seourl(self, ids):
        if not ids:
            return "('null')"
        result = "('product_id=" + \
            "','product_id=".join([str(id) for id in ids]) + "')"
        return result
    ...
    def category_to_in_condition_seourl(self, ids):
        if not ids:
            return "('null')"
        result = "('category_id=" + \
            "','category_id=".join([str(id) for id in ids]) + "')"
        return result

    def get_category_parent(self, category_id):
        query = {
            'type': 'select',
            'query': "SELECT * FROM  _DBPRF_categories WHERE categories_id = " + to_str(category_id)  # noqa
        }
    ...
    def import_category_parent(self, convert_parent):
        parent_exists = self.get_map_field_by_src(
            self.TYPE_CATEGORY, convert_parent['categories_id'], convert_parent['code'])  # noqa
        if parent_exists:
    ...
    def get_country_id(self, code, name):
        ...
        countries = self.get_connector_data(self.get_connector_url('query'), {
                                            'query': json.dumps(countries_query)})  # noqa
        if not countries or countries['result'] != 'success' or not countries['data']:  # noqa
            return 0
    ...
    def get_state_id(self, code, name):
        ...
        zones = self.get_connector_data(self.get_connector_url('query'), {
                                        'query': json.dumps(zones_query)})
    ...
    def _cook_shipping_address_by_billing(self, shipping_address, billing_address):  # noqa
            ...
            if key in {'country', 'state'}:
                for child_key, child_value in shipping_address[key].items():
                    if not shipping_address[key][child_key]:
                        shipping_address[key][child_key] = billing_address[key][child_key]  # noqa
            else:
    ...
    def categories_default_seo(self, category, categories_ext):
        ...
        category_url = get_list_from_list_by_field(
            categories_ext['data']['URIs'], 'cat_id', category['categories_id'])  # noqa
    ...
    def products_default_seo(self, product, products_ext):
        ...
        type_seo = self.SEO_301
        category_url = get_list_from_list_by_field(
            products_ext['data']['URIs'], 'cat_id', product['products_id'])
